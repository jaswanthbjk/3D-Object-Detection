...
def get_instance_seg_v1_net(point_cloud, one_hot_vec,
    ...
    global_feat = tf.compat.v1.concat([global_feat, tf.expand_dims(tf.expand_dims(one_hot_vec, 1), 1)], axis=3)
    global_feat_expand = tf.compat.v1.tile(global_feat, [1, num_point, 1, 1])
    concat_feat = tf.compat.v1.concat(axis=3, values=[point_feat, global_feat_expand])
