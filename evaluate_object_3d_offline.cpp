...
enum CLASSES{CAR=0, PEDESTRIAN=1, BICYCLE=2};
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  indices = getEvalIndices(result_dir + "/data/");
  ...
  for (int32_t i=0; i<indices.size(); i++) {
    ...
    string file_name;
    // printf("I am here");
    // sprintf(file_name,"%s.txt",indices.at(i));
    file_name = indices.at(i);
    // printf("Filename-%s", indices.at(i));
    cout<<file_name<<endl;
