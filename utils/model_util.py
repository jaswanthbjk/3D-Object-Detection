...
def extract_scenes(dataset):
    scenes = []
    for file in dataset:
        scenes.append(int(file.split('_')[1]))
    return scenes


def perf_split(dataset):
    eval_files = random.sample(dataset, 50)
    train_files = [x for x in dataset if x not in eval_files]
    val_files = random.sample(eval_files, 25)
    test_files = [x for x in eval_files if x not in val_files]
    return train_files, val_files, test_files


def num_scenes(scenes):
    lyftd = load_train_data()
    n_samples = 0
    for scene in scenes:
        start_sample_token = lyftd.scene[scene]['first_sample_token']
        sample_token = start_sample_token
        while sample_token != "":
            sample_record = lyftd.get('sample', sample_token)
            n_samples += len(sample_record['anns'])
