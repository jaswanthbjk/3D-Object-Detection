...
def conv_bn(x, filters, trainable, activation='relu'):
    conv = tfp.layers.Convolution2DFlipout(filters=filters, kernel_size=1, strides=1, padding="valid",
                                           kernel_divergence_fn=None)
    conv = add_kl_weight(conv, train_size=240485)
    x = conv(x)
    if activation == 'relu':
...
def dense_bn(x, filters, trainable):
    dense = tfp.layers.DenseFlipout(filters, kernel_divergence_fn=None)
    dense = add_kl_weight(dense, train_size=240485)
    x = dense(x)
