...
def draw_gt_boxes3d(gt_boxes3d, fig, name = 0, color=(1, 1, 1), line_width=1, draw_text=True, text_scale=(1, 1, 1),
                    color_list=None):
    ...
    for n in range(num):
        ...
        if color_list is not None:
            color = color_list[n]
        if draw_text: mlab.text3d(b[4, 0]+1, b[4, 1]+1, b[4, 2]+1, '%s' % name, scale=0.75, color=color, figure=fig)
        for k in range(0, 4):
