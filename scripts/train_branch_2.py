...
from F_PointNet_red import Frustum_Pointnet_Model
...
model,_ = Frustum_Pointnet_Model()
print(len(model.layers))
# model.save("my_model.h5")

# loss_funcs = {"InsSeg_out": class_loss_fn,
#               "center_out": "huber_loss",
#               "heading_scores": class_loss_fn,
#               "size_scores": class_loss_fn}
# loss_weights = {"InsSeg_out": 2.0, "center_out": 8.0, "heading_scores": 5.0, "size_scores": 10.0}
...
checkpoint_path = "/scratch/jbandl2s/kitti/training/checkpoints/cp-79-{epoch:04d}"
...
def make_iterator(dataset):
    ...
    with K.get_session().as_default() as sess:
        while True:
            ...
            #             "size_scores": size_class_label_pl, "heading_residual_label": heading_residual_label_pl,
            #             "size_residual_label": size_residual_label_pl}
...
# with open("./training_2/model.json", "w") as json_file:
#     print('Saved_model_architecture')
#     json_file.write(model_json)
metrics = Metrics(val_data=itr_valid, step=487, batch_size=32)
model.load_weights('./model_ckpts/cp-0180.ckpt')
num_freezing_layers = 79
for layer in model.layers[0:num_freezing_layers]:
    layer.trainable = False
for layer in model.layers[num_freezing_layers:len(model.layers)+1]:
    init_layer(layer)
