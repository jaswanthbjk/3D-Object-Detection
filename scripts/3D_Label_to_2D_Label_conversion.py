...
def get_2d_boxes(sample_data_token: str, visibilities: List[str]) -> List[OrderedDict]:
    ...
    sd_rec = level5data.get('sample_data', sample_data_token)
    ...
    cs_rec = level5data.get('calibrated_sensor', sd_rec['calibrated_sensor_token'])
    pose_rec = level5data.get('ego_pose', sd_rec['ego_pose_token'])
    camera_intrinsic = np.array(cs_rec['camera_intrinsic'])

    # Get all the annotation with the specified visibilties.
    ann_recs = [level5data.get('sample_annotation', token) for token in s_rec['anns']]
    ...
    for ann_rec in ann_recs:
        ...
        box = level5data.get_box(ann_rec['token'])
...
def main(args):
    ...
    print("Generating 2D reprojections of the Level5dataenes dataset")

    # Get tokens for all camera images.
    sample_data_camera_tokens = [s['token'] for s in level5data.sample_data if (s['sensor_modality'] == 'camera') and
                                 s['is_key_frame']]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataroot', type=str, default=r'F:\\LyftDataset\\v1.01-train', help="Path where Level5data is saved.")
    parser.add_argument('--version', type=str, default='\\v1.01-train', help='Dataset version.')
